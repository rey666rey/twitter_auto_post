...
def choose_file(quantity: int) -> list[str]:
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(PHOTO_DIR, f)) and f.lower().endswith(extensions)
    ]

    if not files:
        raise FileNotFoundError(f"No media files found in {PHOTO_DIR} with extensions {extensions}")
